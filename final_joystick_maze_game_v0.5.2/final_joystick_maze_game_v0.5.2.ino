...
【更新文档】
[V0.5.2] 2019.08.23 增加joystickA_offset、joystickB_offset变量，开机自动获取遥杆偏移值，并且在控制函数中进行相应的补偿调整。
[V0.5.1] 2019.08.18 增加defaultAngleA、defaultAngleB变量，可以通过更改该变量来调整舵机初始位置。
...
int defaultAngleA=94;//请更新此变量，手动调整默认舵机水平角度
...
{
  Serial.begin(9600);
  joystickA_offset = 512-analogRead(A0);//开机自动获取并记录当前摇杆偏移值
  joystickB_offset = 512-analogRead(A1);//需要放在servoSetup()之前，因为该函数会用到
  sevSegSetup();
  servoSetup();
...
//  servoA.write(defaultAngleA);
//  servoB.write(defaultAngleB); 
  servoA.write(map(analogRead(A0)+joystickA_offset,0,1023,defaultAngleA-10,defaultAngleA+10));//开机时的舵机角度和开始游戏时的一样,而不是直接转到defaultAngle
  servoB.write(map(analogRead(A1)+joystickB_offset,0,1023,defaultAngleB-10,defaultAngleA+10));//保证开始游戏后舵机会不轻微转动
}
